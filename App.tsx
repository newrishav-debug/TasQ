...
const App: React.FC = () => {
  ...
  const handleAcceptTask = useCallback(async (task: Task) => {
    const acceptedTask = { ...task, status: TaskStatus.ACCEPTED };
    try {
      const response = await fetch(API_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        setTasks(prev => [...prev, acceptedTask]);
      }
    } catch (err) {
  ...
  const handleToggleComplete = useCallback(async (id: string) => {
    ...
    const newStatus = task.status === TaskStatus.COMPLETED ? TaskStatus.ACCEPTED : TaskStatus.COMPLETED;
  ...
  const handleUpdateTaskPriority = useCallback(async (id: string, urgency: Level, importance: Level) => {
    const task = tasks.find(t => t.id === id);
    if (!task) return;

    const updatedTask = { ...task, aiUrgency: urgency, aiImportance: importance };
